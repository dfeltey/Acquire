...
(require "admin.rkt" 
         (except-in "basics.rkt" hotel? shares-order? cash?) 
         (except-in "board.rkt" tile? board? xtile? tile->xexpr) 
         (except-in "state.rkt" state? player? xstate? state->xexpr) 
         "Lib/io.rkt" "Lib/log.rkt" "Lib/xml.rkt")

;; Need to move all this to a generic wrapper sort of thing...
;; To handle it cleanly for requires among different files
(require/typed xml 
               [#:opaque XExpr xexpr?])
...
(define-type (Parser p) (Any -> (Option p)))
...
(: boardhotel-parser (Parser Hotel))
...
(: order-parser (Parser Shares-Order))
(define order-parser
...
(: booleans-parser (Parser (Listof Boolean)))
...
(: players-parser (Parser (Listof Player)))
...
(: end-parser (Parser (Pairof State Score)))
(define end-parser 
  (xml-parser 
